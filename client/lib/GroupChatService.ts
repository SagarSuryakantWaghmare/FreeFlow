...
class GroupChatService {
  ...
  private initializeStompClient(): void {
    console.log('Initializing STOMP client with URL:', `${this.baseUrl}/ws`);
    
    const socket = new SockJS(`${this.baseUrl}/ws`);
    
    // Add event listeners to SockJS for debugging
    socket.onopen = () => {
    ...
    this.stompClient.onDisconnect = (frame) => {
      console.log('Disconnected from STOMP server:', frame);
    };
